...
<div class="head"> Measurement View / Edit</div>
...
$item='Measurement';
$limit=20;
$query="select * from tbl_measure order by  id asc";
...
<input type="submit" value="Refresh"  onclick="window.location='admin.php?action=page&icon=measureview&t=viewMeasurements';" class="refreshbutton"/>  
...
echo "<div class=warning>  Check atleast a measurement you want to change. </div>";
...
<input name="cancel" type="submit" value="Cancel" onclick="window.location='admin.php?action=page&icon=measureview&t=Measureviews';" class="clearbutton" /> 
</div>
  <?php
}// else
...
$id=$codeno;
$ofname = ucfirst($catname[$name]);
$query = "update tbl_measure set  name= '$ofname' where id='$id'";
$result=mysqli_query($mysqli, $query) or die(mysqli_error());
echo $item="<div class=success> Measurement Succussfullly Done <font color=red>".strtoupper($catname[$name])."</font></div>";
}
...
{
// get the names of the items
$g="select * from tbl_offence where  id='$value'";
$gr=mysqli_query($mysqli, $g) or die(mysqli_error());
...
<?php
echo "Are  you Sure you want to Delete id ( ".$value ." ), "." Measurement Name ( ". $gname ." )<br>";
echo "</div>";
...
<h4 class="sets"> 
     ...
     &nbsp;|| &nbsp;
      <a href="admin.php?action=page&icon=measureview&t=viewMeasurements" class="currentx">
      NO </a>
