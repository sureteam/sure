...
<div class="head"> Species View / Edit</div>
...
$item='Specie';
$limit=20;
$query="select * from tbl_species order by  id asc";
...
<input type="submit" value="Refresh"  onclick="window.location='admin.php?action=page&icon=speciesview&t=viewSpecies';" class="refreshbutton"/>  
...
echo "<div class=warning>  Check atleast a specie you want to change. </div>";
...
$g="select * from tbl_species where  id='$value'";
...
?>
    <tr>
      ...
      class="ids"  autocomplete="off"/></td>
    
      <td bgcolor="<?php echo $row_color;?>"><input name="name[]"  id="name[]" type="text" value="<?php echo $catname;?>" class="textbox"   autocomplete="off">      </td>    
    </tr>
...
<input name="cancel" type="submit" value="Cancel" onclick="window.location='admin.php?action=page&icon=speciesview&t=Speciesviews';" class="clearbutton" /> 
</div>
  <?php
}// else
...
$id=$codeno;
$sname = ucwords($catname[$name]);
$query = "update tbl_species set  name= '$sname' where id='$id'";
$result=mysqli_query($mysqli, $query) or die(mysqli_error());
echo $item="<div class=success> Specie Succussfullly Done <font color=red>".strtoupper($catname[$name])."</font></div>";
}
...
<?php
echo "Are  you Sure you want to Delete id ( ".$value ." ), "." Specie Name ( ". $gname ." )<br>";
echo "</div>";
...
<h4 class="sets"> 
     ...
     &nbsp;|| &nbsp;
      <a href="admin.php?action=page&icon=speciesview&t=viewSpecies" class="currentx">
      NO </a>
